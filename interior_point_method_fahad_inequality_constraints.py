...
def Interior_point_method(f,A,b,Q,c,x,lambdaa,s,rho,mu,mu_min):
    ...
    ############ Find alpha0  #########################################
        ...
        I = np.argwhere(dxk < 0)       
        J = I[:,0]
        zz = -np.divide(x[J],dxk[J])

        II = np.argwhere(dsk < 0)
       
        JJ = II[:,0]       
        zzz = -np.divide(s[JJ],dsk[JJ]) 
        junk = np.concatenate((zz,zzz,[[1.0]]),0)
...
# input the data from IP problem 02
# def problem_no1():
#     A = np.array([[5., -4, 13, -2, 1],[1,-1, 5, -1, 1]])  # impiut the data matrix
#     b = np.array([[20.],[8]]) # right side 
#     m = A.shape[0]
#     n = A.shape[1]

# input the data from IP problem 01
def problem_no1():
    A = np.array([[7.,10,1,0,0,0],[3.,5,0,1,0,0],[3.,2,0,0,1,0],[2.,5,0,0,0,1]])  # impiut the data matrix
    b = np.array([[6300.],[3600],[2124],[2700]]) # right side 
    m = A.shape[0]
    n = A.shape[1]
    ...
    ## Phase number 2
    Q = np.zeros((n,n))
    #c = np.array([[1.],[6],[-7],[1],[5]])
    c = np.array([[-10.],[-9], [0],[0],[0],[0]])
    x, lambdaa, s = Interior_point_method(problem_fun,A,b,Q,c,x,lambdaa,s,rho,mu,mu_min)
